...
Note that because we have to get the defects associated with stories associated with a release, we're going to have to get the appropriate oids of defects and stories that CURRENTLY meet the release rule and trace them through their history.  So, we won't see if a defect changes from one Release to another, but more importantly, it means we hit wsapi first, then lookback.
